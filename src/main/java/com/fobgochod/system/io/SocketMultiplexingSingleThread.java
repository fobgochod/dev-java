...
/**
 * 多路复用器单线程
 * Java NIO java.nio.channels.Selector
 * poll epoll 不同底层实现
 * <p>
 * 加上jvm参数 指定具体的多路复用器，默认(epoll)
...
public class SocketMultiplexingSingleThread {
    ...
    // linux多路复用器 (select poll epoll kqueue) nginx event{}
    int port = 9090;
    ...
    public void initServer() {
        try {
            ...
            // select poll *epoll 优先选择：epoll 但是可以使用 Djava.nio.channels.spi.SelectorProvider指定
    ...
    public void start() {
        ...
        try {
            while (true) {
                ...
                参数可以带时间：
                    没有时间或者0 => 阻塞，
                    有时间设置一个超时，可以在外部线程通过selector.wakeup()，结果返回0
                ...
                while (selector.select() > 0) {
                     ...
                     * NIO：自己对着每一个fd调用系统调用，浪费资源，，这里(多路复用)是不是调用了一次select方法，知道具体的那些可以R/W了？
                     * socket分两种：
                     *      1. listen，等待accept
                     *      2. 通信 R/W
                     */
